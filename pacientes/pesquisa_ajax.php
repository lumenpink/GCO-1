<?php
    ...
    */
    ...
    function em_debito($codigo) {
        $query = mysql_query("SELECT DISTINCT(vo.codigo_paciente), tp.* FROM pacientes tp INNER JOIN v_orcamento vo ON tp.codigo = vo.codigo_paciente WHERE data < '".date('Y-m-d')."' AND pago = '0' AND confirmado = '1' AND baixa = '0' AND tp.codigo = ".$codigo." ORDER BY `nome` ASC");
...
<?php
    ...
    if($_GET['campo'] == 'debito') {
        $sql = "SELECT DISTINCT(vo.codigo_paciente), tp.* FROM pacientes tp INNER JOIN v_orcamento vo ON tp.codigo = vo.codigo_paciente WHERE data < '".date('Y-m-d')."' AND pago = '0' AND confirmado = '1' AND baixa = '0' ORDER BY `nome` ASC";
    }
